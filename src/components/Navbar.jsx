    ...
    <header style={{ background:'#000', color:'#fff', borderBottom:'1px solid #222' }}>
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'14px 20px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        ...
        <nav className="nav-desktop" style={{ display:'flex', gap:24, alignItems:'center' }}>
          ...
          <NavLink to="/kpis" style={({isActive}) => ({ color:'#fff', textDecoration:'none', fontSize:14, fontWeight:500, opacity: isActive ? 1 : 0.7, transition:'opacity 0.2s' })}>KPIs</NavLink>
          <NavLink to="/clientes" style={({isActive}) => ({ color:'#fff', textDecoration:'none', fontSize:14, fontWeight:500, opacity: isActive ? 1 : 0.7, transition:'opacity 0.2s' })}>Clientes</NavLink>
          <NavLink to="/ofertas" style={({isActive}) => ({ color:'#fff', textDecoration:'none', fontSize:14, fontWeight:500, opacity: isActive ? 1 : 0.7, transition:'opacity 0.2s' })}>Ofertas</NavLink>
          <NavLink to="/social" style={({isActive}) => ({ color:'#fff', textDecoration:'none', fontSize:14, fontWeight:500, opacity: isActive ? 1 : 0.7, transition:'opacity 0.2s' })}>📱 Social</NavLink>
      ...
      {open && (
        <div className="nav-mobile" style={{ borderTop:'1px solid #222', background:'#000' }}>
          <div style={{ maxWidth:960, margin:'0 auto', padding:'8px 12px', display:'grid', gap:8, justifyItems:'stretch' }} onClick={()=>setOpen(false)}>
            ...
            <NavLink to="/clientes" className={({isActive})=>`mobile-link ${isActive?'active':''}`} style={({isActive})=>({ display:'block', width:'100%', textAlign:'center', textDecoration:'none', padding:'12px 14px', borderRadius:999, border:'1px solid #222', background: isActive?'#000':'#fff', color: isActive?'#fff':'#000', margin:'0 0' })}>Clientes</NavLink>
            <NavLink to="/ofertas" className={({isActive})=>`mobile-link ${isActive?'active':''}`} style={({isActive})=>({ display:'block', width:'100%', textAlign:'center', textDecoration:'none', padding:'12px 14px', borderRadius:999, border:'1px solid #222', background: isActive?'#000':'#fff', color: isActive?'#fff':'#000', margin:'0 0' })}>Ofertas</NavLink>
            <NavLink to="/social" className={({isActive})=>`mobile-link ${isActive?'active':''}`} style={({isActive})=>({ display:'block', width:'100%', textAlign:'center', textDecoration:'none', padding:'12px 14px', borderRadius:999, border:'1px solid #222', background: isActive?'#000':'#fff', color: isActive?'#fff':'#000', margin:'0 0' })}>📱 Social</NavLink>
