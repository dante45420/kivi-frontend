    ...
    <header style={{ background:'#000', color:'#fff' }}>
      <div style={{ maxWidth:960, margin:'0 auto', padding:'10px 12px', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
        ...
        <nav className="nav-desktop" style={{ display:'none', gap:10, alignItems:'center' }}>
          ...
          <NavLink to="/admin/kpis" className={({isActive})=>`chip ${isActive?'active':''}`} style={{ background:'#fff', color:'#000', textDecoration:'none' }}>📊 KPIs B2B</NavLink>
      ...
      {open && (
        <div className="nav-mobile" style={{ borderTop:'1px solid #222', background:'#000' }}>
          <div style={{ maxWidth:960, margin:'0 auto', padding:'8px 12px', display:'grid', gap:8, justifyItems:'stretch' }} onClick={()=>setOpen(false)}>
            ...
            <NavLink to="/admin/kpis" className={({isActive})=>`mobile-link ${isActive?'active':''}`} style={({isActive})=>({ display:'block', width:'100%', textAlign:'center', textDecoration:'none', padding:'12px 14px', borderRadius:999, border:'1px solid #222', background: isActive?'#000':'#fff', color: isActive?'#fff':'#000', margin:'0 0' })}>📊 KPIs B2B</NavLink>
