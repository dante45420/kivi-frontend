...
export default function VueltaReconocimientoModal({ open, onClose, products, onSuccess }) {
  ...
  async function handleSave() {
    ...
    Object.entries(prices).forEach(([productId, data]) => {
      // Precio base (sin variante) - SOLO si no hay variantes o si se llenó explícitamente
      if (data.price && parseFloat(data.price) > 0) {
        const hasVariantPrices = data.variants && Object.values(data.variants).some(v => v.price && parseFloat(v.price) > 0)
        
        // Solo agregar precio base si NO hay variantes con precios
        if (!hasVariantPrices) {
          pricesArray.push({
        ...
        Object.entries(data.variants).forEach(([variantId, variantData]) => {
          if (variantData.price && parseFloat(variantData.price) > 0) {
            pricesArray.push({
              ...
              unit: variantData.unit || data.unit,
              markup_percentage: 20,
              min_qty: 1.0
            })
          }
    ...
    try {
      ...
      alert(`✓ Se guardaron ${pricesArray.length} precio(s) exitosamente`)
      onSuccess()
      onClose()
    } catch (e) {
      alert('Error: ' + e.message)
  ...
  ).map(([cat, prods]) => [cat, prods.sort((a, b) => a.product_name.localeCompare(b.product_name))])
  .sort((a, b) => {
    const order = { fruta: 1, verdura: 2, otro: 3 }
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{
                          ...
                          alignItems: 'center',
                          gap: '8px',
                          borderBottom: hasVariants ? '1px solid #f5f5f5' : 'none'
                        }}>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            ...
                            {hasVariants && (
                              <div style={{ fontSize: '11px', color: '#88C4A8', marginTop: '2px', fontWeight: 600 }}>
                                {productVariants[product.product_id].length} variantes disponibles
                              </div>
                            )}
                          </div>
                          {hasVariants && (
